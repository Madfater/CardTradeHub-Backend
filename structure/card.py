...
def AddStoreCard(Store_ID:int, data:dict):
    if sql.countTable(f"Store where ID = {Store_ID}") == 0:
        return "Store not found"
    id = sql.getMaxId("StoreCard") + 1
    StoreCard_arg = [id,data["price"],data['status'],data['quantity'],data['accard_ID'],Store_ID]
    sql.command(sql.insert("StoreCard",StoreCard_arg))
    store.updateStoreTime(getStore(id))
    return id

# 更新 StoreCard
def updateStoreCard(Card_ID:int, data:dict):
    if sql.countTable(f"StoreCard where ID = {Card_ID}") == 0:
        return "Card not found"
    if sql.command(f"select Store_ID from StoreCard where ID =  {Card_ID}")[0][0] != data['user_id']:
        return "no access"
    store.updateStoreTime(getStore(Card_ID))
    ...
    sql.command(f"update StoreCard set {','.join(condition)} where ID = {Card_ID}")
    return "updated"

# 下架 StoreCard
def removeStoreCard(Card_ID:int,data:dict):
    if sql.countTable(f"StoreCard where ID = {Card_ID}") == 0:
        return "Card not found"
    if sql.command(f"select Store_ID from StoreCard where ID =  {Card_ID}")[0][0] != data['user_id']:
        return "no access"
    store.updateStoreTime(getStore(Card_ID))
    sql.command(f"DELETE FROM Order_to_Card_Table WHERE Card_ID = {Card_ID}")
    sql.command(f"DELETE FROM Card_to_Cart_TableID WHERE Card_ID = {Card_ID}")
    sql.command(f"DELETE FROM StoreCard WHERE ID = {Card_ID}")
    return "removed"
